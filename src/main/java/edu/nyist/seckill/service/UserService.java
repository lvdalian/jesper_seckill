...
import edu.nyist.seckill.exception.GlobalException;
import edu.nyist.seckill.mapper.UserMapper;
import edu.nyist.seckill.redis.RedisService;
import edu.nyist.seckill.redis.UserKey;
import edu.nyist.seckill.result.CodeMsg;
import edu.nyist.seckill.util.CodeUtil;
import edu.nyist.seckill.util.MD5Util;
import edu.nyist.seckill.util.UUIDUtil;
...
public class UserService {
    ...
    public String login(HttpServletRequest request, HttpServletResponse response, LoginVo loginVo) {
        if (!CodeUtil.checkVerifyCode(request)) {
            throw new GlobalException(CodeMsg.VERIFY_CODE);
        }
        if (loginVo == null) {
